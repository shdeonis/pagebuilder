                    ...
                    switch (fieldConfig.type) {
                        ...
                        case 'select': {
                            ...
                            if (!$input.get(0).tomselect) {
                                ...
                                var optionsData = $input.attr('data-options');
                                if (optionsData) {
                                    try {
                                        settings.options = JSON.parse(optionsData);
                                        settings.valueField = "value";
                                        settings.labelField = "text";
                                        settings.searchField = "text";
                                    } catch (e) {
                                        console.error("Ошибка парсинга data-options:", e);
                                    }
                                }

                                var ts = new TomSelect($input.get(0), settings);
                                $input.data('tomselect', ts);
                            }
